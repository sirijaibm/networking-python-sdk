...
def fix_test_file(file_path):
    ...
    # Check if already fixed - look for our specific skip message
    if 'pytest.skip("ibm_cloud_sdk_core or ibm_cloud_networking_services not installed", allow_module_level=True)' in content:
    ...
    while i < len(lines):
        ...
        # Check if this is the first SDK import line we need to wrap
        if not found_imports and ('from ibm_cloud_sdk_core' in line or 
            ...
            while i < len(lines):
                ...
                if 'from ibm_cloud_sdk_core' in current_line or 'from ibm_cloud_networking_services' in current_line:
